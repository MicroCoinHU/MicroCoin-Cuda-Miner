...
void grindNonces()
{
	...
	*nonceOut = 0;
	...
	cudaMemcpyAsync(midStatemobj, midstateInternal, 32, cudaMemcpyHostToDevice, cudastream);
	cudaMemcpyAsync(nonceOutmobj, nonceOut, 4, cudaMemcpyHostToDevice, cudastream);
	cudaMemcpyAsync(blockHeadermobj, remainingHeader, 68, cudaMemcpyHostToDevice, cudastream);
	nonceGrindc << < 128, 1024, blocksize, cudastream >> > (blockHeadermobj, midStatemobj, nonceOutmobj, hextarpow);
	cudaMemcpyAsync(nonceOut, nonceOutmobj, 4, cudaMemcpyDeviceToHost, cudastream);
